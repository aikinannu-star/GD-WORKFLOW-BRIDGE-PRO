import type { ApiClient } from '@gd-workflow-bridge-pro/api-sdk';
import { normalizeSdkError } from '../errors.js';

export async function runMarketplaceDemo(client: ApiClient) {
  ...
  if (Array.isArray(products.items) && products.items.length > 0) {
    ...
    products.items.slice(0, 3).forEach((product: any, index: number) => {
