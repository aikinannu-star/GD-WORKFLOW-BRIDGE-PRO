...
async function main() {
    ...
    if (Array.isArray(products.items) && products.items.length > 0) {
        ...
        const productData = product && typeof product === 'object' && 'data' in product ? product.data : product;
    ...
    if (config.tenantId) {
        ...
        const tenantData = tenant && typeof tenant === 'object' && 'data' in tenant ? tenant.data : tenant;
