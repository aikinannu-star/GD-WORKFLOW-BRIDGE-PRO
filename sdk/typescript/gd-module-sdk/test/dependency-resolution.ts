...
async function run() {
  ...
  const pluginA: any = await client.registerPlugin({ name: 'DepA', version: '0.0.1', description: 'dependency A', author: 'dev' });
  ...
  const pluginB: any = await client.registerPlugin({ name: 'DepB', version: '0.0.1', description: 'dependency B', author: 'dev' });
  ...
  // Create tenant
  const tenantRes: any = await client.createTenant('Dep Tenant', `dep-${Date.now()}.local`);
  const tenantId: string = (tenantRes && (tenantRes.tenant || tenantRes).id) as string;
  ...
  const res: any = await client.installPlugin(pluginB.id, tenantId, '1.0.0', { auto_install_dependencies: true });
  console.log('installed with auto-deps:', res.status);

  // Verify both installs exist
  const installsB: any = await client.listPluginInstalls(pluginB.id, tenantId);
  const installsA: any = await client.listPluginInstalls(pluginA.id, tenantId);
